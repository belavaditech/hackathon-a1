...
export default class StrategyBusinessChart extends LightningElement {
    @api strategyId;
    @track businessData = [];
    @track reasons = [];
    @track chart;
    ...
    wiredStrategy({ error, data }) {
        if (data) {
            try {
                let cleanedJsonString = data.Business_data__c
                ...
                this.businessData = parsedData.businessImpacts;
                this.reasons = this.businessData.map(item => ({
                    aspect: item.type,
                    reason: item.reason
                }));
    ...
    renderChart() {
        if (!this.isChartJsInitialized || !this.businessData.length) {
            return;
        }
        ...
        const labels = this.businessData.map(item => item.type);
        const scores = this.businessData.map(item => item.score);

...
export default class BusinessPromptFlow extends LightningElement {
    @track promptResult;
    ...
    wiredStrategy({ error, data }) {
        if (data) {
            this.businessData = data.Business_data__c;
    ...
    startFlow() {
        ...
        if (flowComponent) {
            flowComponent.startFlow('Process_business_data', this.inputVariables);
        }
    }

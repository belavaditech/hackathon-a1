...
export default class ManilaPromptFlow extends LightningElement {
    @track promptResult;
    ...
    wiredStrategy({ error, data }) {
        if (data) {
            this.emotionalData = data.Emotional_data__c;
            this.description = data.Description__c;
            this.error = undefined;
        } else if (error) {
            this.error = error.body.message;
            this.emotionalData = undefined;
    ...
    startFlow() {
        ...
        if (flowComponent) {
            flowComponent.startFlow('Process_emotional_data', this.inputVariables);
        }
    }
